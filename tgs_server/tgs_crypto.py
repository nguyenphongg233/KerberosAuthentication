"""
TGS Crypto: Hệ thống mã hóa của Ticket Granting Server
...
class TGSCryptoEngine:
    """Engine mã hóa của TGS"""
    ...
    def encrypt_dict(data: Dict[str, Any], key: str) -> str:
        ...
        return TGSCryptoEngine.encrypt(json_str, key)
    
    @staticmethod
    def decrypt_dict(ciphertext_hex: str, key: str) -> Dict[str, Any]:
        """Giải mã thành dictionary"""
        plaintext = TGSCryptoEngine.decrypt(ciphertext_hex, key)
        if not plaintext:
            return {}
        try:
