...
\hline
\texttt{client/credential\_cache.py} & Lưu TGT, service ticket và session key trong credential cache nhị phân MIT ccache v4 \\
\hline
...
\begin{lstlisting}[style=textcode]
PBKDF2-HMAC-SHA1(password, principal_salt, 1000 iterations)
\end{lstlisting}
...
Keytab chứa principal, realm, kvno, enctype và key của service dưới định dạng nhị phân big-endian. Application Server đọc file này khi start và dùng key để decrypt service ticket.

Client lưu credential cache nhị phân {\bf MIT ccache v4} tại:

\begin{lstlisting}[style=textcode]
client/krb5cc_demo
\end{lstlisting}

Cache chứa TGT, service ticket, session key và metadata thời hạn dưới định dạng nhị phân chuẩn. Nếu ticket đã quá \texttt{endtime}, client tự bỏ cache entry khi đọc.
...
\begin{itemize}
    ...
    \item Rate limiting và account lockout cho các cuộc tấn công đoán mật khẩu.
\end{itemize}
...
\hline
KDF & PBKDF2-HMAC-SHA1, 1000 iterations + derive-random (RFC 3962) \\
\hline
Database & SQLite \\
\hline
Keytab/cache & Định dạng nhị phân MIT Keytab v2 \& ccache v4 \\
\hline
\end{tabularx}
\end{table}
...
Hoặc chạy kịch bản kiểm thử liên Realm tự động:
...
\begin{enumerate}
    ...
    \item Thêm rate limiting và cơ chế lockout tài khoản khi đoán sai mật khẩu nhiều lần.
...
So với mô phỏng đơn giản ban đầu, project hiện tại đã tiến sát các tiêu chuẩn RFC 4120, RFC 3961, và RFC 3962 nhờ bổ sung principal/realm chuẩn, dẫn xuất khóa PBKDF2-HMAC-SHA1 có salt, định dạng nhị phân chuẩn **MIT Keytab v2** và **MIT ccache v4**, cơ chế **Xác thực chéo Realm (Cross-Realm Trust)**, phân quyền nhóm (RBAC/Authorization Data), đàm phán subkey/seq-number, replay cache bền vững, audit log, outer/inner wire messages dạng ASN.1/DER và thuật toán mã hóa đối xứng AES-CTS kết hợp checksum HMAC-SHA1-96 chuẩn hóa cùng cơ chế Key Usage. Tuy nhiên, nhóm cũng xác định rõ giới hạn: đây vẫn chưa phải Kerberos production hoàn toàn do socket TCP chưa tích hợp TLS/mTLS và chưa tương thích trực tiếp hoàn toàn với nhân hệ thống Active Directory.
