...
\node[box, below=of app] (keytab) {MIT Keytab v2\\ Service long-term key};

\draw[arrow] (client) -- node[left]{AS-REQ / AS-REP} (kdc);
\draw[arrow] (client.east) -- node[above, pos=0.7]{TGS-REQ / TGS-REP} ++(3.2,0) |- (kdc.east);
\draw[arrow] (client.south east) -- node[sloped, above, pos=0.4]{AP-REQ / AP-REP} (app.west);
...
\hline
\texttt{app\_server/service\_server.py} & Application Server, đọc keytab nhị phân, kiểm AP-REQ và trả AP-REP \\
\hline
...
\hline
\texttt{KDC\_HOST}, \texttt{KDC\_PORT} & \texttt{127.0.0.1}, \texttt{4321} & Địa chỉ KDC \\
\hline
...
\State Client tạo \texttt{PA-ENC-TIMESTAMP} gồm \texttt{ctime}/\texttt{cusec}; principal và realm nằm trong \texttt{req-body}
...
\State TGS chọn đúng TGS key theo principal/kvno/enctype rồi decrypt TGT
...
\State TGS kiểm service principal tồn tại và có loại \texttt{service} hoặc \texttt{tgs}
...
\State Client gửi \texttt{Authorization: Negotiate <Base64(AP-REQ DER)>} tới Application Server
\State Server chọn keytab entry theo principal/kvno/enctype và decrypt service ticket
...
\State Server trả \texttt{AP-REP} mã hóa bằng client subkey nếu có, nếu không thì dùng \texttt{Kc\_service}; nội dung chứa lại \texttt{ctime}/\texttt{cusec} của authenticator cùng subkey/sequence number của server
\State Client decrypt \texttt{AP-REP} và xác nhận \texttt{ctime}/\texttt{cusec} khớp để hoàn tất mutual authentication
\end{algorithmic}
\end{algorithm}
...
So với mô phỏng đơn giản ban đầu, project hiện tại đã tiến sát các tiêu chuẩn RFC 4120, RFC 3961, và RFC 3962 nhờ bổ sung principal/realm chuẩn, dẫn xuất khóa PBKDF2-HMAC-SHA1 có salt, định dạng nhị phân MIT Keytab v2 và MIT ccache v4 subset, cơ chế **Xác thực chéo Realm (Cross-Realm Trust)**, phân quyền nhóm (RBAC/Authorization Data), đàm phán subkey/seq-number, replay cache bền vững, audit log, outer/inner wire messages dạng ASN.1/DER và thuật toán mã hóa đối xứng AES-CTS kết hợp checksum HMAC-SHA1-96 chuẩn hóa cùng cơ chế Key Usage. Tuy nhiên, nhóm cũng xác định rõ giới hạn: đây vẫn chưa phải Kerberos production hoàn toàn do socket TCP chưa tích hợp TLS/mTLS và chưa tương thích trực tiếp hoàn toàn với nhân hệ thống Active Directory.
