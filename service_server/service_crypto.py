"""
Service Server Crypto: Hệ thống mã hóa của Service Server
...
class ServiceCryptoEngine:
    """Engine mã hóa của Service Server"""
    ...
    def encrypt_dict(data: Dict[str, Any], key: str) -> str:
        ...
        return ServiceCryptoEngine.encrypt(json_str, key)
    
    @staticmethod
    def decrypt_dict(ciphertext_hex: str, key: str) -> Dict[str, Any]:
        """Giải mã thành dictionary"""
        plaintext = ServiceCryptoEngine.decrypt(ciphertext_hex, key)
        if not plaintext:
            return {}
        try:
